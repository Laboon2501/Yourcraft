# LocalQuestReborn v0.9 最小真实 NPC 生成实验

本轮新增 `RealNpcSpawnService`，但没有真正生成 3D actor。原因是 Brio / AQuestReborn 的真实 actor 生成链路不是一个稳定、独立、低依赖的小接口；直接接入会把 LocalQuestReborn 从“本地任务运行时”变成“依赖 Brio 内部服务图的 actor 控制插件”。当前版本先把接口、UI、生命周期清理和日志打通。
...
- 依赖链太重，不适合直接进入 LocalQuestReborn 的核心插件。
- AGPL 代码不能复制进本项目；本轮只参考接口边界和生命周期思路。

## 当前阻塞点

1. Brio 没有在 LocalQuestReborn 中可直接调用的稳定轻量 spawn API。
...
2. 如果 Brio 没有合适 IPC，新增 `IRealNpcBackend` 接口，让 LocalQuestReborn 支持“无后端 / Brio 后端 / 未来其他后端”。
