...
因此这条路线不能作为 LocalQuestReborn 的“场景物体生成”成功路径。它本质仍然依赖 `ActorSpawnService.CreateCharacter`，只会生成角色 clone。
...
当前实机结果说明这条对象创建层仍会返回 `Npc/Character clone`，并且 `ActorEntity.IsProp=false`。所以在 LocalQuestReborn 里不能把它算作真实 Prop 成功。
