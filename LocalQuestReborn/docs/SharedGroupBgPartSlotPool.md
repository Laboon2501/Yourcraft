...
LocalQuestReborn previously only added the top-level `SharedGroupLayoutInstance` row and did not expose its children as selectable BgPart slots. That meant the BgPart Slot Pool could not find child-only models such as `x6t1_ad_scr00.mdl`.
