...
研究玩家右键/确认键点击真实 EventNpc 时，客户端侧哪些目标对象和 UI addon 状态会变化，为后续 LocalQuestReborn 接管本地多段对话做准备。
...
本地多段对话应由 LocalQuestReborn 自己维护：

1. 原生 NPC 只作为 host / 交互入口。
2. 玩家 target host NPC 后：
   - `/lqr talk` 直接打开 LocalQuestReborn `DialogueWindow`。
...
当当前 target 匹配 host DataId/ObjectIndex，执行 `/lqr talk` 会走 LocalQuestReborn 本地对话。
