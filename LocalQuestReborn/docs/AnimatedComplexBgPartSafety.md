...
Animated or dynamic material models can show as static after recreate because their motion appears to depend on original layout/shared group/event update context, not only the `BgObject` model path and transform. LocalQuestReborn does not yet recreate that controller chain.
