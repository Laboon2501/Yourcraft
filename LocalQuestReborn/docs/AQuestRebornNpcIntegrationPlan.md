# LocalQuestReborn v1.1 AQuestReborn 真实 NPC 集成路线分析

当前结论：Brio 在本机只暴露了 `ApiVersion` 类 IPC，没有公开可用的 Spawn/Create/Delete Actor IPC。因此 LocalQuestReborn 不能只靠 Brio IPC 生成真实 NPC。AQuestReborn 的做法是直接引用 Brio assembly 和一组相关项目，在插件进程内取得 Brio 服务并调用 `ActorSpawnService.CreateCharacter`。
...
这说明 LocalQuestReborn 不能只保存 `npcId -> object index` 就结束。至少要有一个运行态对象负责验证 `ICharacter.IsValid()`、区域切换清理和 actor 生命周期。
...
LocalQuestReborn 如果要复刻真实 NPC，至少需要以下路线之一：
...
3. **单独后端插件**：写一个小型 Brio companion plugin 暴露 LocalQuestReborn 需要的 IPC，避免主插件直接依赖 Brio 内部类。

不建议 fork 或复制 AQuestReborn/Brio 的实现到 LocalQuestReborn。AQuestReborn 是 AGPL，Brio 也有自身许可证和内部服务图，直接复制会带来许可证和维护风险。

## LocalQuestReborn 最小复刻需要的类/接口
...
这样 LocalQuestReborn 的任务运行时不会被 Brio/AQuestReborn 的重依赖直接污染，真实 NPC 能作为可选实验后端逐步推进。
