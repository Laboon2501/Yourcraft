...
`ResourceHandle.Type.Category` is the most useful runtime readback source. For a selected BgPart, Yourcraft now reads `ModelResourceHandle->Type.Category` directly and shows it in the UI.
...
Yourcraft now displays for the selected instance:
