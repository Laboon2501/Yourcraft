# Yourcraft v1.0 Brio IPC 调研结果

本轮目标是让 `RealNpcSpawnService` 尝试通过 Brio IPC 生成本地可见 actor，同时不破坏现有虚拟 NPC overlay 和任务流程。

## 本机检查

开发环境中从 `%AppData%\XIVLauncher` 和 `%AppData%\XIVLauncherCN` 递归查找 Brio 相关配置/文件时没有得到可用结果。由于 Dalamud 插件是否安装、是否加载最好以游戏运行时的 `IDalamudPluginInterface.InstalledPlugins` 为准，Yourcraft 现在在运行时通过 `BrioNpcBridgeService.StatusText` 显示：
...
如果 Brio IPC 3.0 改动了这些签名，Yourcraft 会捕获异常，写入 Dalamud log，并在实验 NPC 页签显示最后一次 Spawn 错误信息。当前还没有游戏内异常堆栈可记录；下一步需要在启用 Brio 3.0 的环境中点击“生成选中 NPC”确认具体失败点。
...
如果用户当前安装的 Brio 版本没有上述 IPC，Yourcraft 不会复制 Brio 源码。替代路线有三种：
...
3. 参考 AQuestReborn 架构：Yourcraft 保持任务系统和 NPC 数据，真实 actor 层单独做后端插件或可选模块。优点是核心任务系统不被重依赖污染。
