...
Yourcraft 当前已经能把 ENpc/BNpc 解析为两类外观：
...
```text
Actor实例 / 应用 NPC 外观
  -> Yourcraft.Services.RealNpcSpawnService.ApplyNpcAppearance(runtimeId)
     Yourcraft/Services/RealNpcSpawnService.cs:293
    -> Yourcraft.Services.AppearanceApplyService.ApplyNpcAppearance(npc, actor)
       Yourcraft/Services/AppearanceApplyService.cs:32
      -> ApplyGameNpc(...)
         Yourcraft/Services/AppearanceApplyService.cs:96
        -> GameNpcAppearanceResolver.Resolve(appearance)
           Yourcraft/Services/GameNpcAppearanceResolver.cs:20
        -> GameNpcAppearanceApplyService.TryApplyModelChara(actor, resolution, out reason)
           Yourcraft/Services/GameNpcAppearanceApplyService.cs
          -> BrioHumanoidAppearanceApplyService.TryApplyHumanoid(actor, resolution, out reason)
             Yourcraft/Services/BrioHumanoidAppearanceApplyService.cs
...
Yourcraft 本轮新增 `GlamourerStateApplyService` 只做 wrapper 探测和签名显示。原因是现有 `Customize + Equipment` 还不是 Glamourer state base64；强行拼 base64 会变成猜 schema。
...
Yourcraft 的 `GameNpcResolvedAppearance.Customize` 对应 Brio `ActorAppearance.FromENpc` 中的 ENpcBase 字段：
...
- 从 Yourcraft 自己的 `Customize + Equipment` 构造 Glamourer state base64。
